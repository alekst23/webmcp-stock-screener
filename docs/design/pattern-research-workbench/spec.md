...
| WebMCP tool count always visible                                | the page loads                                                                                           | the researcher looks at the page header                                                             | it shows the total number of tools the app defines (e.g. "11 WebMCP tools available"), regardless of whether the current browser can connect via WebMCP or how many are currently unlocked by workflow state                                                                                                     |
| WebMCP tool names available to agents, hidden from the human UI | the page loads                                                                                           | an agent (or anything else reading the page's HTML source, not the rendered view) inspects the page | the full list of tool names is present as an HTML comment, with a short preface explaining what they are and how to call them via `document.modelContext`; nothing renders visibly to the human researcher — the header still shows only the count, per the existing "WebMCP tool count always visible" scenario |
