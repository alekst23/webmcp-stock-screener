...
| WebMCP tool counts always visible                        | the page loads                                                                                              | the researcher looks at the page header                                                             | it shows two counts: the total number of tools the app defines (e.g. "11 WebMCP tools defined") and the number currently callable by an agent (e.g. "5 available"). The defined count never varies with browser support or workflow state                                                                                                                                                                                                              |
...
| Bridge fails to connect                                  | the browser supports `document.modelContext` but registration rejects (network error, malformed descriptor) | the page loads                                                                                      | the available count is 0 and the header reports the failure distinctly from the unsupported-browser case, with the underlying error surfaced to the browser console for diagnosis — not a blank, stale, or silently-successful header                                                                                                                                                                                                                  |
| Available count tracks agent-driven availability changes | a connected bridge and an agent tool call whose side effects unlock or retire tools (feature #10)           | the tool call completes                                                                             | the available count updates to match what is currently registered, without a page reload; the defined count is unchanged                                                                                                                                                                                                                                                                                                                               |
...
- **Human-driven workflow actions do not refresh the available tool
  count.** The registered tool set is only re-evaluated when an agent's
  tool call completes. A researcher acting through the visible UI can
  unlock or retire the same tools (feature #10) — clearing panels, for
  instance — and the header's available count keeps showing the previous
  set until an agent calls something or the page reloads. Pre-existing;
  narrowing the "Available count tracks agent-driven availability
  changes" scenario above documents the gap rather than closing it.
