# T-0002-3: Timeline UI + remove raw state dump
...
1. Each log entry visibly shows its actor as "Human" or "Agent" (per
   T-0002-1's `actor` field), the action, a human-readable summary, and a
   timestamp.
...
- `ActivityFeed.svelte`: each `<li>` gains an actor badge, sourced from a
  ...
  inaccurate (this feed already receives both actors once T-0002-1
...
- No change to `store.ts`, `activity.ts`, `register.ts`, or
  `ChartToolbar.svelte` — those are T-0002-1/T-0002-2's surface.
...
EPIC-0003). A full visual design system — this is a structural fix to the
