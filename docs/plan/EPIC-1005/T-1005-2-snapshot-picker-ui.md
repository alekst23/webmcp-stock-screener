...
scenarios, wiring T-1005-1's `saveSnapshot`/`loadSnapshot`/`deleteSnapshot`/
...
- New component `SnapshotPicker.svelte` (implementation-phase work, not a
  ...
  into `workspaceStore` via `.set(...)` (already normalized by T-1005-1).
...
- `src/lib/workspace/snapshots.ts` (T-1005-1) — `saveSnapshot`,
