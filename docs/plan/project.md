...
| EPIC-0025: Server-side screener evaluation endpoint | epic | **implemented, both tickets Done, not yet reviewed/closed** — `/at-epic-run` completed 2026-09-03 (`ced62a5`, `85261d0`) | `epic/EPIC-0025-server-side-screener-evaluation` | T-0025-1: `UniverseSpec.sectors` any-of filter via a new `SectorCatalog` Protocol + `unrecognized_sectors()`; `field.price.change_pct` resolved vectorized in `PanelPriceSeriesPort.get_series`. T-0025-2: new `PortScreenerRunEngine` (mirrors `PortBacktestEngine`) wired into `POST /api/screener/run`; response is one flat `ScreenerRunResult` (`status: complete\|refused\|valid`), matching `BacktestResultsResponse`'s convention; empty-universe/unrecognized-sector are named blocking refusals, zero-matches-after-evaluation is a normal complete run. Full catalog-backed parameter-range validation explicitly out of scope (no Python catalog registry exists — EPIC-1008 is TS-only), documented not fabricated. **334 backend tests passing (was 320, +24 new)**, mypy/black/isort/flake8 clean, mutation-checked on the empty-universe refusal and `change_pct` resolution paths. Design/test gates skipped per project convention (ACs were already solid); implementation plans written into both ticket docs first. Verified: commits present on the epic branch (not just the delegate's worktree), worktree clean. **This is the real fix for EPIC-0020's documented `run_screener` → `empty_universe` gap.** Ready for `/at-epic-review`/`/at-epic-close` — needs user go-ahead (interactive-only skill). |
| EPIC-0026: Agent screener loop | epic | **in progress** — `/at-epic-run` launched 2026-09-03 as part of Wave 1 (parallel with EPIC-0025/EPIC-0027) | `epic/EPIC-0026-agent-screener-loop` | 3 tickets (atomic `define_screener` tool; `search_catalog` registration; evaluation-port + composition-root wiring). Triaged from issue #26, 2026-09-03 (commit `f19be80`). Replaces the current 5-call mutation sequence (`create_screener`/`set_screener_universe`/`edit_filter_tree`/`set_screener_ranking`/`validate_screener`) with one atomic, fully-validated call; wires the real HTTP evaluation port against EPIC-0025's endpoint via the existing fake-port override seam (does not block on EPIC-0025 landing first); registers exactly the MVP tool surface in the composition root, **removing** (not commenting out) everything else. |
| EPIC-0027: Screener widget & drag-to-chart | epic | **implemented, both tickets Done, not yet reviewed/closed** — `/at-epic-run` completed 2026-09-03 (`20241db`, `dbe1c76`, `a238e7f`) | `epic/EPIC-0027-screener-widget-drag-to-chart` | T-0027-1: real `filter_builder` panel body reading `WorkspaceDocument.screenerId` via `readScreener`, renders universe/filter-tree/ranking, explicit empty state, no edit controls (`src/lib/screener/panel/`, `src/lib/screener/registry/`). Used a fixture screener (`writeScreener`/`repository.put`) rather than waiting on EPIC-0026's `define_screener`, matching the shape it will produce once landed. T-0027-2: native HTML5 drag-and-drop from `ResultsTableRow.svelte` onto `PanelContainer.svelte`, routed through new `createChartFromDrop`/`bindPanelSourceFromDrop` wrappers in `panelController.ts` calling the exact `createPanel`/`bindPanelSource` use cases, tagged `actor: 'human'`. **Design correction made mid-implementation** (documented in the ticket): AC1's "the exact cell dropped on" means the panel's top-left anchor, not a literal 1x1 footprint — a bare 1x1 rect fails the chart's 2x2 minimum size. **Known gap for review:** dragged rows have no `exchange`/`assetType` data (no reference-data source exists anywhere in this project — same pre-existing gap `resolveTicker.ts` already documents), so `resultRowDrag.ts` mints a provisional `exchange: 'XUNK'`, `assetType: 'equity'` instead of resolving real values. **Full suite green: 3043 passed, 2 skipped, 1 todo** (46 new tests across 9 new files), typecheck clean across 771 files. Design/test gates skipped per project convention. Verified: commits present on the epic branch, worktree clean. Ready for `/at-epic-review`/`/at-epic-close` — needs user go-ahead (interactive-only skill). |
