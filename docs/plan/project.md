...
|------|------|--------|--------|-------|
| EPIC-1006: Workspace, revisions & common tool contract | epic | **launching (`/at-epic-run`, this session)** | `epic/EPIC-1006-…` merged to main (spec) | 8 tickets, zero code as of launch. Foundation: envelope, revisions, idempotency, undo, operation registry; owns `get_canvas_state` (renamed from `get_workspace`). Reconciliations #1/#2 resolved by working assumption (see Decisions Log) so it doesn't have to stop for them mid-run. |
...
| EPIC-1008: Discovery & catalog | epic | **closing (`/at-epic-close`, this session)** | `epic/EPIC-1008-discovery-and-catalog` (4 commits ahead of main) | All 7 tickets landed 2026-09-01. Frontend 224 tests pass (was 117), svelte-check 0 errors, backend 60/5. All new files; no existing source touched. Not wired into the live page — composition root unowned. Reconciliation #4 (OHLCV bars port) resolved by working assumption to unblock this close. |
...
| EPIC-0016: AWS re-platform | epic | **in progress** | `epic/EPIC-0016-aws-replatform` (`ef237dd`) | 11 tickets. Spec written and all open questions settled 2026-09-01; `main` merged in so the branch carries EPIC-0013's code. Wave 1 (T-0016-1 image, T-0016-2 health, T-0016-3 object store, T-0016-4 Terraform foundation) running in parallel worktrees. T-0016-4 is authorized to apply against the live account; T-0016-11 (decommission Render) stays user-gated. |
