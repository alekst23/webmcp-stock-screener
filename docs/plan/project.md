...
| EPIC-1007: Panel system | epic | **implementation in flight, resumed after a rate-limit interruption** (2026-09-02) | `epic/EPIC-1007-panel-system` (consolidating on worktree branch `epic-1007-consolidation`) | 9 tickets. **Landed:** T-1007-1 (panel entity, panel-kind registry, source/renderer contract registry), T-1007-2/T-1007-8 (grid layout geometry, fixed 6x4). **In progress:** T-1007-3 (link graph — solution approach written, implementation resumed). Remaining: T-1007-4, T-1007-5, T-1007-6, T-1007-9. Agent hit the session rate limit (reset 5:10am) and was resumed with full context at 09:24. 14 tools; owns panel-kind registry and source/renderer contract registry — the extension point EPIC-1010/1011 plug renderer contracts into. |
...
| EPIC-1009: Screener core | epic | **implementation in flight, resumed after a rate-limit interruption** (2026-09-02) | `epic/EPIC-1009-screener-core` | 10 tickets. **Landed:** T-1009-1 (screener definition model, typed filter tree). T-1009-3/4/5 sub-agents were mid-flight when the rate limit hit (reset 5:10am); resumed at 09:24 with instruction to check their worktrees for salvageable committed work before restarting. Owns the `run_id`/pinned-run contract EPIC-1010 blocks on, and must store per-match filter-node evaluated value + pass/fail during evaluation so `explain_result` is a lookup rather than a re-run. Working assumption for pinned-run retention: eviction is an explicit error, TTL unset, policy left pluggable. |
...
| EPIC-1011: Chart tools | epic | **implementation in flight, resumed after a rate-limit interruption** (2026-09-02) | `epic/EPIC-1011-chart-tools` | 9 tickets. **Landed but not yet consolidated onto the epic branch:** T-1011-1 (chart domain model + captured-setup contract), T-1011-2 (study calculation engine), T-1011-3 (OHLCV bars port + HTTP/in-memory adapters) — done in per-ticket worktrees, orchestrator resumed at 09:24 to merge them in and continue T-1011-4 through T-1011-9. 3 tools + chart-renderer contract registered into EPIC-1007's registry (T-1011-4/T-1011-5's standalone-tool framing is superseded by the 2026-09-01 reconciliation — design docs win). Owns the captured-setup contract EPIC-1012 blocks on. |
...
| EPIC-1013: Safety layer (preview & apply) | epic | **implementation in flight, resumed after a rate-limit interruption** (2026-09-02) | `epic/EPIC-1013-safety-preview-apply` | 6 tickets. **Landed:** T-1013-1 (preview/apply domain contracts), T-1013-2 (non-mutating batch evaluation over the operation registry), T-1013-3 (structured workspace diff), T-1013-4 (preview store, stable IDs + expiry) — furthest along of the four Wave 2 epics. Remaining: T-1013-5 (atomic apply — must land with a test demonstrating the rollback/failure path), T-1013-6 (tool wiring). 2 tools; generic over operations that do not exist yet. Handlers are explicitly *not* assumed pure (2026-09-01 decision). |
