...
| EPIC-0025: Server-side screener evaluation endpoint | epic | **implemented, both tickets Done, merged to `dev`** — `/at-epic-run` completed 2026-09-03 (`ced62a5`, `85261d0`), merged to `dev` 2026-09-04 | `epic/EPIC-0025-server-side-screener-evaluation` | T-0025-1: `UniverseSpec.sectors` any-of filter via a new `SectorCatalog` Protocol + `unrecognized_sectors()`; `field.price.change_pct` resolved vectorized in `PanelPriceSeriesPort.get_series`. T-0025-2: new `PortScreenerRunEngine` (mirrors `PortBacktestEngine`) wired into `POST /api/screener/run`; response is one flat `ScreenerRunResult` (`status: complete\|refused\|valid`), matching `BacktestResultsResponse`'s convention; empty-universe/unrecognized-sector are named blocking refusals, zero-matches-after-evaluation is a normal complete run. Full catalog-backed parameter-range validation explicitly out of scope (no Python catalog registry exists — EPIC-1008 is TS-only), documented not fabricated. **334 backend tests passing (was 320, +24 new)**, mypy/black/isort/flake8 clean, mutation-checked on the empty-universe refusal and `change_pct` resolution paths. Design/test gates skipped per project convention (ACs were already solid); implementation plans written into both ticket docs first. Verified: commits present on the epic branch (not just the delegate's worktree), worktree clean. **This is the real fix for EPIC-0020's documented `run_screener` → `empty_universe` gap.** Merged to `dev` (pushed to `origin/dev`); not yet reviewed via `/at-epic-review`/closed via `/at-epic-close`, and `dev` has not been merged to `main` — needs user go-ahead (interactive-only skills). |
| EPIC-0026: Agent screener loop | epic | **Done — all 6 tickets, merged to `dev`** | `epic/EPIC-0026-agent-screener-loop` | Triaged from issue #26, 2026-09-03 (commit `f19be80`). T-0026-1 (`define_screener`) and T-0026-2 (`search_catalog` + sector enumeration) landed first. **T-0026-3 was originally one bundled ticket** (evaluation port + result rows + retention + composition root) — a first attempt widened `ScreenerMatch` without updating callers (17 typecheck errors) and was reverted 2026-09-04. Re-split into four independent tickets and re-run via `/at-epic-run`: **T-0026-3** widens `ScreenerMatch`/`get_screener_results` to a full instrument ref (symbol/exchange/assetType/name), mutation-checked; **T-0026-4** adds `HttpScreenerEvaluationPort` against EPIC-0025's real endpoint, wired as the composition root's default (replacing the honest-unavailable in-browser engine as the live path); **T-0026-5** actually registers `define_screener`/`run_screener`/`get_canvas_state` in the composition root (previously unregistered or commented out since EPIC-1015's "chart-demo trim") and deletes the old dead 5-tool legacy screener surface, proven by a real end-to-end integration test (`define_screener` → `run_screener` → `get_screener_results` → `create_panel`) against the live composition root, not just the override seam; **T-0026-6** bounds `PinnedRunStore` to the most-recently-pinned run (was unbounded), keeps the in-browser engine (still a real fallback default on `runScreener.ts`'s public constructor, documented why), and syncs `docs/architecture/tool-surface-status.md`. One integration fix made directly by the orchestrator merging waves: `HttpScreenerEvaluationPort`'s match mapper was missing the new ref fields (built in a sibling worktree before T-0026-3 existed). **Full suite green on the epic branch: 3054 tests, typecheck 0 errors.** Merged to `dev` and pushed; also fixed a second cross-epic seam on `dev` directly — EPIC-0027's `resultRowDrag.ts` was still minting a provisional `exchange: 'XUNK'` placeholder, now wired to the row's real ref. **The MVP loop (EPIC-0025 backend + EPIC-0026 agent tools + EPIC-0027 UI) is now genuinely reachable end to end on `dev`** — not yet merged to `main`. |
| EPIC-0027: Screener widget & drag-to-chart | epic | **implemented, both tickets Done, merged to `dev`** — `/at-epic-run` completed 2026-09-03 (`20241db`, `dbe1c76`, `a238e7f`), merged to `dev` 2026-09-04 | `epic/EPIC-0027-screener-widget-drag-to-chart` | T-0027-1: real `filter_builder` panel body reading `WorkspaceDocument.screenerId` via `readScreener`, renders universe/filter-tree/ranking, explicit empty state, no edit controls (`src/lib/screener/panel/`, `src/lib/screener/registry/`). Used a fixture screener (`writeScreener`/`repository.put`) rather than waiting on EPIC-0026's `define_screener`, matching the shape it will produce once landed. T-0027-2: native HTML5 drag-and-drop from `ResultsTableRow.svelte` onto `PanelContainer.svelte`, routed through new `createChartFromDrop`/`bindPanelSourceFromDrop` wrappers in `panelController.ts` calling the exact `createPanel`/`bindPanelSource` use cases, tagged `actor: 'human'`. **Design correction made mid-implementation** (documented in the ticket): AC1's "the exact cell dropped on" means the panel's top-left anchor, not a literal 1x1 footprint — a bare 1x1 rect fails the chart's 2x2 minimum size. **Known gap for review:** dragged rows have no `exchange`/`assetType` data (no reference-data source exists anywhere in this project — same pre-existing gap `resolveTicker.ts` already documents), so `resultRowDrag.ts` mints a provisional `exchange: 'XUNK'`, `assetType: 'equity'` instead of resolving real values. **Full suite green: 3043 passed, 2 skipped, 1 todo** (46 new tests across 9 new files), typecheck clean across 771 files. Design/test gates skipped per project convention. Verified: commits present on the epic branch, worktree clean. Merged to `dev` (pushed to `origin/dev`); not yet reviewed via `/at-epic-review`/closed via `/at-epic-close`, and `dev` has not been merged to `main` — needs user go-ahead (interactive-only skills). |
...
1. **Resolved 2026-09-03** — see Decisions Log. EPIC-0025/EPIC-0026/EPIC-0027
   are the "what's next" answer: a real backend evaluation endpoint, the
   agent-facing `define_screener` loop wired to it, and the human-facing
   screener widget + drag-to-chart. All three triaged, on their own branches,
   ready for `/at-epic-run`, designed to build in parallel.
...
| ~~The new WebMCP surface's composition root has no owner~~ — **RESOLVED 2026-09-03**, confirmed by EPIC-0020 (merged `7d35f95`, PR #21). `/workbench`'s composition root now wires `WorkspaceRepository`/ID sequencer/idempotency cache/revision service/change history/`PinnedRunStore` once and flips `WORKBENCH_TOOLS_ENABLED`/`SCREENER_TOOLS_ENABLED` on; `/workbench` is live on `main`. | EPIC-1015 (cutover blocks on this existing), all of EPIC-1006-1014 (their tools stay unreachable without it) | 2026-09-01 | Confirmed by EPIC-1008's epic review (independent [WIRING] audit grepped every EPIC-1006/1015 ticket, found none). EPIC-1006's own tool-surface ticket (T-1006-8) registers only its own tools; EPIC-1015's cutover tickets presuppose the new surface already exists rather than assembling it. Needs an explicit new ticket (likely under EPIC-1006 or EPIC-1015) before EPIC-1015 can cut over. See `docs/architecture/new-webmcp-surface.md`. |
...
|------|----------|-----------|
| 2026-09-03 | Backlog item 1 resolved: pursue a real screener evaluation adapter via three new epics (EPIC-0025/26/27) triaged from issues #25/26/27, rather than reconsidering the EPIC-1015 cutover first | These three epics directly close the `empty_universe` wiring gap EPIC-0020 documented (real backend endpoint → agent tool loop → human-facing widget). EPIC-1015 cutover stays paused per the 2026-09-02 decision — untouched by this. |
...
## Last Run (2026-09-04, /at-project-go Wave 1 + merge to dev)

- **Trigger:** User asked to finish now and merge all work to `dev`
  (no `dev` branch existed yet — created from `main`).
- **T-0026-3 status:** its delegate's background sub-agent produced a
  half-written change (widened `ScreenerMatch` to require new fields
  without updating any caller/fixture/composition-root — 17 typecheck
  errors) and was **reverted**, not merged. `epic/EPIC-0026-agent-screener-loop`
  stays at T-0026-1 + T-0026-2 (`3615a02`). T-0026-3 (real HTTP evaluation
  port + composition-root wiring, now unblocked since EPIC-0025's real
  endpoint has landed) is an **open follow-up**, not started.
- **Merged to `dev`** (created from `main` at `4f5031d`, pushed to
  `origin/dev`): EPIC-0025 (full, both tickets), EPIC-0027 (full, both
  tickets), EPIC-0026 (partial — T-0026-1/T-0026-2 only). All three
  merges were clean, no conflicts.
- **Verified on the final `dev` tip, not just individual epic branches:**
  typecheck 0 errors across 781 files; frontend 3089 tests passing (2
  skipped, 1 todo); backend 334 real tests passing, 9 pre-existing
  failures confirmed unrelated (EPIC-1015 T-1015-8 deployment-cutover
  placeholders, not implemented yet, and a commit-hygiene check —
  present before this run's changes).
- **Not done:** `dev` was not merged to `main`, no PR opened, epics not
  formally reviewed/closed (`/at-epic-review`/`/at-epic-close` are
  interactive-only and weren't run). EPIC-0027's known gap (drag creates
  a provisional `exchange: 'XUNK'` placeholder, no real reference-data
  source exists anywhere in this project) is unresolved.
- **Next session should:** decide whether to open a `dev` -> `main` PR
  (with review) or keep iterating on `dev` first; pick up T-0026-3 as a
  fresh ticket now that EPIC-0025's real endpoint exists to wire against.

## Last Run (2026-09-03, /at-project-go assessment)

- **Trigger:** `/at-project-go`, no argument.
- **Found, not made by this run:** three new epics — EPIC-0025, EPIC-0026,
  EPIC-0027 — already triaged from issues #25/#26/#27 onto their own
  branches (commits `c41675b`/`f19be80`/`ac14e4b`), each with `_epic.md` +
  ticket docs, none yet reflected in this plan. Also found uncommitted
  local changes to `docs/architecture/README.md` and
  `docs/architecture/tool-surface-mvp.md` (a 7-tool MVP rewrite, dated
  2026-09-03 21:42, predating the triage commits) — not made by this run,
  left untouched pending user input. Also found stale worktree debris from
  prior runs: several clean worktrees on already-merged branches, plus one
  dirty worktree (`t-1006-3`/`agent-ab5c9b313859dd4b5`, uncommitted domain
  files superseded by what actually landed on `main` from EPIC-1006) —
  left untouched.
- **Shipped:** Plan updated with the three new epics, Backlog item 1
  resolved (Decisions Log), and the composition-root Blocker marked
  resolved (EPIC-0020 already fixed it, wasn't reflected here).
- **Next:** see wave plan presented to the user this session.
