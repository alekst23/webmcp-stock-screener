...
| EPIC-0026: Agent screener loop | epic | **partial — T-0026-1/T-0026-2 Done, merged to `dev`; T-0026-3 not started (reverted)** | `epic/EPIC-0026-agent-screener-loop` (tip `3615a02`) | Triaged from issue #26, 2026-09-03 (commit `f19be80`). **Landed:** atomic `define_screener` tool (T-0026-1) and `search_catalog` registration + enumerated field values (T-0026-2) — replaces the old 5-call mutation sequence (`create_screener`/`set_screener_universe`/`edit_filter_tree`/`set_screener_ranking`/`validate_screener`) with one atomic, fully-validated call. **Not landed:** T-0026-3 (real HTTP evaluation port + composition-root wiring) — its delegate's background sub-agent widened `ScreenerMatch` to require new fields without updating any caller/fixture/composition-root (17 typecheck errors), so it was reverted rather than merged, 2026-09-04. **Now unblocked** since EPIC-0025's real endpoint has landed for real — open follow-up, needs a fresh implementation pass. |
