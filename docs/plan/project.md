...
1. **Issue #20 — wire the composition root for a minimal live demo** (current
   priority, user decision 2026-09-02: MVP over full cutover). Flip
   `WORKBENCH_TOOLS_ENABLED`/`SCREENER_TOOLS_ENABLED`, give screener +
   workbench-core tools a shared `WorkspaceRepository`/`PinnedRunStore` with
   `registerPanelTools.ts`'s existing instances (each tool group builds its
   own today — confirmed by codebase-impact analysis 2026-09-02), decide
   run→panel binding (auto-bind vs. explicit `bind_panel_source`), and write
   the first integration test proving `create_screener` → ... → `run_screener`
   → visible in `results_table` on `/workbench`. Chart/similarity/backtest/
   alerts/watchlist/followup tool groups stay off. Not yet triaged into an
   epic — next step.
2. **EPIC-1015 cutover** — paused pending issue #20 and reconsideration of
