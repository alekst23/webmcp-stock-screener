...
| EPIC-0025: Server-side screener evaluation endpoint | epic | **in progress** — `/at-epic-run` launched 2026-09-03 as part of Wave 1 (parallel with EPIC-0026/EPIC-0027) | `epic/EPIC-0025-server-side-screener-evaluation` | 2 tickets (T-0025-1 universe/field resolution incl. a new percent-change-over-N-sessions field; T-0025-2 stateless `POST /screener/run` endpoint). Triaged from issue #25, 2026-09-03 (commit `c41675b`). Design: `docs/design/screener-core/` (spec already exists — no design gate needed). Composes the existing, currently-uncalled `backend/domain/filter_evaluation.py` evaluator with universe narrowing and the historical price panel. **This is the real fix for EPIC-0020's documented `run_screener` → `empty_universe` gap.** Nothing here touches the frontend. Blocks EPIC-0026 in intent (its real evaluation port targets this endpoint) but the epic doc states EPIC-0026 is independently testable via a fake port, so both run in parallel. |
