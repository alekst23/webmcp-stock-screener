...
| EPIC-1015: Legacy surface cutover | epic | **user approved "full cutover" 2026-09-02, not yet launched** | `epic/EPIC-1015-legacy-surface-cutover` (not yet created) | 8 tickets (`docs/plan/EPIC-1015/`), 0/8 have a Solution Approach — same design-gate-skip pattern as EPIC-1007/1009/1011/1013 should apply (`docs/design/legacy-surface-cutover/spec.md` exists as the substantive design). **User explicitly confirmed the full-cutover option** (audit → parity check → migrate routes onto one page → backend reconciliation → delete legacy tool surface → delete legacy workspace model → docs → deploy verification) when asked to choose between full cutover, route-migration-only, or visual-parity-first. Orchestrator was about to dispatch `/at-epic-run EPIC-1015` when the session was interrupted for closeout — **nothing launched, no branch/worktree created, no code touched.** See "Next session should" in the latest Last Run entry for the review-gate plan (hold before the two deletion waves, T-1015-5/6, for orchestrator review — this epic is not purely additive like EPIC-1007-1014 were). Design-parity context to hand to T-1015-2/T-1015-3: user's reference mockup shows a screener-logic panel (left), a chart with a pinned reference setup + studies list (center), a "Similar Setups" sidebar (right, maps to EPIC-1012's `similar_opportunities` panel kind — registered but **not** in the current 3-panel default seed), a watchlist panel and an alert-draft card (bottom right — **EPIC-1014's watchlist/alert tools have no registered panel kind today**, tool-only), and the results table (bottom). The current default seed (`seedDefaultWorkspace` in `panelController.ts`) only places `filter_builder`/`results_table`/`chart` — richer default composition is real, undone work if visual parity with the mockup is the goal, not just capability-parity. |
