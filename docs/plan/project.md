...
| EPIC-0027: Screener widget & drag-to-chart | epic | **in progress** — `/at-epic-run` launched 2026-09-03 as part of Wave 1 (parallel with EPIC-0025/EPIC-0026) | `epic/EPIC-0027-screener-widget-drag-to-chart` | 2 tickets (read-only body for the `filter_builder` panel; drag a results row onto the canvas to create/bind a chart). Triaged from issue #27, 2026-09-03 (commit `ac14e4b`). Scoped down during triage: the issue's proposed seed-layout change was dropped as unnecessary (`run_screener`'s auto-bind already creates the results panel on first run; the empty-grid-canvas hotfix already keeps a fresh workspace's seed minimal). Buildable and testable against a fixture workspace document independently of EPIC-0026 landing first. |
