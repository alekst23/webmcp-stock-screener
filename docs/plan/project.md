...
| EPIC-1007: Panel system | epic | specced, grid model resolved | `epic/EPIC-1007-panel-system` (recreated 2026-09-02, not yet pushed — spec-only commit) | 8 tickets (T-1007-8 added). 14 tools; owns panel-kind registry and source/renderer contract registry. **2026-09-02 (`/at-project-design`):** user specified the grid concept explicitly — fixed, non-scrolling 6-column x 4-row grid (24 cells), always exactly filling the viewport. Resolved spec Open Question 1 (was "12-column, unbounded-row"). New "grid is full" rejection scenario added (auto-placement can now genuinely fail, which it couldn't under the unbounded assumption). T-1007-2's AC2 ("rows unbounded") superseded — see its note in `docs/plan/EPIC-1007/_epic.md` — by new T-1007-8, which also extends T-1007-6's rendering AC for the non-scrolling viewport-fill requirement. Zero implementation code still; not yet launched via `/at-epic-run`. |
