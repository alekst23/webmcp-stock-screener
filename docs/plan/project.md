...
| EPIC-1015: Legacy surface cutover | epic | **launched 2026-09-02, audit-only wave in progress** | `epic/EPIC-1015-legacy-surface-cutover` | 8 tickets (`docs/plan/EPIC-1015/`). Per this session's explicit user decision, launched **scoped to T-1015-1 (retirement inventory) and T-1015-2 (capability-parity check) only** — a background agent is producing both documents and will stop and report the go/no-go verdict rather than proceeding to T-1015-3+. No deletion, no code changes in this wave; both tickets are audit/analysis deliverables. **Next session/orchestrator step: review the parity matrix and the three known hard cases (temporal multi-step pattern parity, `measure`/`splitInstances` drop, progressive tool availability) with the user before authorizing T-1015-3 onward** — especially the two deletion waves (T-1015-5/6), which need orchestrator review per the existing review-gate plan. Design-parity context to hand to whoever picks up T-1015-3: user's reference mockup shows a screener-logic panel (left), a chart with a pinned reference setup + studies list (center), a "Similar Setups" sidebar (right, maps to EPIC-1012's `similar_opportunities` panel kind — registered but **not** in the current 3-panel default seed), a watchlist panel and an alert-draft card (bottom right — **EPIC-1014's watchlist/alert tools have no registered panel kind today**, tool-only), and the results table (bottom). The current default seed (`seedDefaultWorkspace` in `panelController.ts`) only places `filter_builder`/`results_table`/`chart` — richer default composition is real, undone work if visual parity with the mockup is the goal, not just capability-parity. |
| EPIC-0017: Engine correctness fixes (`within` bounds + `to_date` clamping) | fix | **triaged 2026-09-02 from issue #17, implementation in progress** | `epic/EPIC-0017-engine-correctness-unvalidated-within-bo` | Single ticket (T-0017-1), no epic overhead. Two bugs in the legacy pattern engine (`backend/infra/pandas_engine.py`, `backend/domain/models/pattern.py`): unvalidated `within` bounds on non-anchor setup steps, and completed instances whose non-anchor steps resolve after `to_date`. Scoping decision (interactive, this session): fix now in the legacy engine rather than holding for EPIC-1015, since it's live until that epic actually retires it; correct the `to_date` bug per the issue's own recommendation (documented behavior change). A background agent is implementing it now. |
