...
| EPIC-1010: Results & explain | epic | **implementation in flight** (2026-09-02, `/at-project-go` background agent) | `epic/EPIC-1010-results-and-explain` (created from `main` by the agent) | 8 tickets, all Open at launch. 2 tools + table-renderer contract registered into EPIC-1007's registry. Consumes EPIC-1009's `PinnedRunStore`/`ScreenerRun`/`nodeEvaluations` contract, now on `main`. |
...
| EPIC-1012: Similarity search | epic | **merged to `main`** (local merge, no PR, 2026-09-02) | ~~`epic/EPIC-1012-similarity-search`~~ (merged; branch kept, worktree cleaned up) | **Done: all 8 tickets landed.** Frontend: 1973/1973 tests, typecheck clean across 548 files. Backend: 198/198 tests (5 pre-existing skips). Similarity feature/scoring contract + pandas-based engine reusing the existing panel-wide `groupby`/`rolling` technique (Python by design, per the ticket — see `docs/plan/EPIC-1012/T-1012-2-similarity-engine.md`), the `similar_opportunities` panel kind, HTTP search/explain API, `find_similar_setups`/`explain_similarity`/`compare_setups` tools, wired behind `SIMILARITY_TOOLS_ENABLED`. Backend wiring is additive-only (`backend/domain/errors.py` +3 exception subclasses, `backend/main.py` +25 lines registering the router). **Real gap found and worked around, not fixed:** `defaultPanelKinds.ts`'s unconditional placeholder registration has no unregister/replace path in `PanelRegistry` (EPIC-1007), so any epic's real panel-kind registration conflicts with it in the live registry — this epic worked around it locally with test-scoped registries rather than editing EPIC-1007's files. See Blockers — needs a real fix before EPIC-1015 wires multiple real panel kinds into one live registry. |
...
| `defaultPanelKinds.ts`'s unconditional placeholder registration has no unregister/replace path in EPIC-1007's `PanelRegistry` | EPIC-1007, EPIC-1010, EPIC-1011, EPIC-1012 (every epic registering a real panel kind) | 2026-09-02 | Found independently by EPIC-1012's agent while implementing T-1012-4/6/7/8. `PanelRegistry` has no method to unregister or replace an already-registered kind, so a sibling epic's real panel-kind registration collides with the placeholder registration `defaultPanelKinds.ts` performs unconditionally. Worked around locally per-epic with test-scoped registries rather than touching EPIC-1007's files — not a fix, a deferral. With EPIC-1010's `results_table` and EPIC-1011's `chart` and EPIC-1012's `similar_opportunities` all now merged and needing to coexist in one live `PanelRegistry` at runtime (not just in isolated tests), this needs a real fix — likely an `unregisterPanelKind`/`replacePanelKind` method, or making `defaultPanelKinds.ts`'s registration conditional — before EPIC-1015 cutover wires the composition root together. |
