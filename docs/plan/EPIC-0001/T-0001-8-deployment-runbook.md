# T-0001-8: Deployment Runbook

**Audience**: a human with a credit card and a browser -- this is the part
of T-0001-8 an autonomous coding agent cannot do (creating real hosting
accounts, clicking through real dashboards). See
[`docs/plan/EPIC-0001/T-0001-8-deploy-ops-mock.md`](T-0001-8-deploy-ops-mock.md)
...
epic's mock-data-backed backend/frontend (`epic/EPIC-0001-pattern-research-workbench`
or later, once T-0001-8's work merges into it).

## What you're deploying

- **Backend**: FastAPI (`backend/`), serving the T-0001-1 synthetic mock
...
4. Render will prompt for the two env vars marked `sync: false` in
   ...
   - `EODHD_API_KEY` -- see [`backend/.env.example`](../../../backend/.env.example)
     ...
     actually calls EODHD yet (that's T-0001-9); set it anyway so the var
...
5. Deploy. Watch the build log: `pip install uv`, then `uv sync --frozen`,
   ...
   `render.yaml`'s header comment for why, and `T-0001-9-real-data-pipeline.md`
...
section originally assumed -- discovered live during T-0001-8 deployment.
...
ticket (T-0001-2's spike endpoint). If T-0001-5's five networked tool
...
check `src/routes/` for what's live) or a real product tool once T-0001-5
...
deployed backend, per T-0001-5's acceptance criteria and
...
hardcoded to `http://localhost:8000` on purpose (T-0001-2 was only ever
proving the round trip works at all), and none of T-0001-5's five
networked tools exist yet to wire `PUBLIC_API_BASE_URL` into. Once
T-0001-5 (tool wiring) and T-0001-6/7 (frontend shell + visualization)
