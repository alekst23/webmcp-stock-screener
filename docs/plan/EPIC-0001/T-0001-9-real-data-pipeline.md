# T-0001-9: Real data pipeline (paid, deferred)
...
Replaces T-0001-1's mock `panel.parquet` with a real one built the same
way — same `PriceBar` schema (T-0001-1/T-0001-3), so nothing downstream
...
during T-0001-8's deployment that Render's persistent disk requires a
...
compute tier — only the storage layer changes from what T-0001-8
originally sketched.
...
**Config vars introduced:** reuses `EODHD_API_KEY` from T-0001-1, but the
underlying EODHD account must be upgraded to the paid EOD Historical Data
plan ($19.99/mo) for this ticket; the free tier used in T-0001-1 is
