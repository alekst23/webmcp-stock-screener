...
5. A full example research session (as in T-0001-5's acceptance criteria)
...
the FastAPI app (uvicorn); the mock panel from T-0001-1 regenerates at
...
- `PUBLIC_API_BASE_URL` (frontend, SvelteKit public env convention) — the
  deployed backend's URL, feeds `ApiClientConfig.baseUrl` (T-0001-5).
...
- **slowapi's `SlowAPIMiddleware` is broken against the FastAPI version
  ...
  whatever routes exist now or get added later (e.g. T-0001-5's five tool
  endpoints), matching AC4's "basic protection... on the public backend"
  intent.
- Only `GET /api/spike/ping` (T-0001-2) is registered in `backend/main.py`
  as of this ticket — T-0001-5's five networked tool endpoints hadn't
  landed yet. The runbook and rate-limit curl example both target that
  endpoint; re-check `backend/main.py` once T-0001-5 lands and prefer one
...
- **Dropped the persistent disk originally planned for the mock panel**
  ...
  This does **not** carry over to T-0001-9: real backfilled data is
  ...
  `T-0001-9-real-data-pipeline.md`.
...
pipeline (T-0001-9) to exist first. Also out of scope: actually carrying
out the deployment (creating the Render/Cloudflare accounts, clicking
through the dashboards) — see
`docs/plan/EPIC-0001/T-0001-8-deployment-runbook.md` for those steps,
